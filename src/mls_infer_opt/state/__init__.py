...
8. model_config 的结构字段只读存在于 TaskContext，**绝不**作为可调 knob（knob 属搜索维度）。
...
- gradient.py  analyze 的一步方向（Gradient / NoMove）——analyze↔generate 共享契约
...
from .gradient import Gradient, NoMove
